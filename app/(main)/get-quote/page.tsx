...
export default function GetQuotePage() {
  ...
  const handleOtpSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!pendingQuote) {
      setOtpError("No pending quote to submit.");
      return;
    }
    ...
    try {
      ...
      const resp = await fetch('/api/quote/notify', {
        method: 'POST',
          ...
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            ...
            <form className="mt-4 space-y-3" onSubmit={handleOtpSubmit}>
              ...
              <button
                type="submit"
                disabled={submitting}
                className="w-full cursor-pointer rounded-full bg-primary px-4 py-2 text-sm font-semibold text-white transition hover:bg-primary/90 disabled:opacity-60"
              >
