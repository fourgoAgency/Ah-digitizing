...
const ContactFormBlue: React.FC = () => {
  ...
  const handleSubmit = async () => {
    if (!formData.name.trim() || !formData.email.trim() || !formData.message.trim()) {
      setFeedbackMessage('Please fill in your name, email, and message before sending.');
      return;
    }
    ...
    try {
      ...
      setFeedbackMessage('Thanks! Your message has been sent successfully.');
      setFormData({ name: '', email: '', phone: '', subject: '', message: '' });
    } catch (error) {
      // eslint-disable-next-line no-console
      console.error('Failed to save contact message:', error);
      setFeedbackMessage('Unable to submit your message right now. Please try again later.');
    } finally {
...
>
  ...
  <div className="space-y-3">
    ...
    <button
      ...
      className="w-full cursor-pointer bg-gray-900 hover:bg-black disabled:cursor-not-allowed disabled:opacity-60 text-white font-semibold py-2.5 rounded-xl transition-colors duration-200 shadow-lg"
    >
