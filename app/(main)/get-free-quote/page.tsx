...
export default function GetQoutePage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch("/api/quote/send-otp", {
        method: "POST",
      ...
      if (!response.ok) {
        throw new Error(result?.error || "Unable to send OTP to email.");
      }
      ...
      setSubmitMessage(error instanceof Error ? error.message : "Unable to send OTP at this time.");
    } finally {
  ...
  const handleOtpSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const verifyResponse = await fetch("/api/quote/verify-otp", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: formData.email.trim().toLowerCase(), otp: normalized }),
      });
      ...
      <section className="relative z-10 mx-auto max-w-7xl">
        ...
        >
          <form className="space-y-6" onSubmit={handleSubmit} noValidate>
            ...
            <div className="overflow-hidden rounded-lg border border-gray-200 bg-white">
              <h2 className="border-b border-gray-200 px-5 py-4 text-xl font-semibold text-primary">
                Continue via WhatsApp? (Optional)
              </h2>
              <div className="p-5">
            ...
            <button
              ...
              className="btn cursor-pointer disabled:cursor-not-allowed disabled:opacity-70"
            >
              {isSubmitting ? "Sending OTP..." : "Submit Quote"}
            </button>
          </form>
