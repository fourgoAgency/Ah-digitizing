    ...
    <section className="rounded-md bg-white p-5 shadow-sm ring-1 ring-slate-100">
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[980px] text-left text-sm">
          <thead><tr className="border-y border-slate-100 text-xs font-medium text-slate-400"><th className="w-10 py-3" /><th className="py-3">Order</th><th className="py-3">Date</th><th className="py-3">Customer</th><th className="py-3">Order Status</th><th className="py-3 text-right">Action</th></tr></thead>
          <tbody>
            {loading ? <tr><td colSpan={6} className="py-16 text-center text-sm text-slate-400">Loading free quote requests...</td></tr> : rows.length > 0 ? rows.map((row) => (
              <tr key={row.id} className="border-b border-slate-100 text-xs text-slate-700 hover:bg-slate-50/70">
                ...
                <td className="py-3"><div className="font-medium text-slate-800">{row.customer}</div><div className="text-[11px] text-slate-500">{row.email}</div></td>
            ...
            )) : <tr><td colSpan={6} className="py-16 text-center text-sm text-slate-400">No free quote requests found.</td></tr>}
          </tbody>
        </table>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 px-4 py-6" role="dialog" aria-modal="true">
          <div className="max-h-[90vh] w-full max-w-5xl overflow-hidden rounded-md bg-white shadow-xl">
            <header className="flex items-start justify-between gap-4 border-b border-slate-100 px-5 py-4">
              <div><p className="text-xs font-semibold uppercase tracking-normal text-slate-400">Quote Detail</p><h3 className="mt-1 text-xl font-bold text-slate-950">{getString(activeQuote, ["fullName", "name"], "Customer")}</h3></div>
            ...
            <div className="max-h-[calc(90vh-78px)] overflow-y-auto px-5 py-5">
              <section className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
                ...
                <div><p className="text-[11px] font-semibold uppercase tracking-normal text-slate-400">Created At</p><p className="mt-1 text-sm font-medium text-slate-800">{formatCreatedAt(getDate(activeQuote.submittedAt) || getDate(activeQuote.createdAt))}</p></div>
              </section>
              ...
              <section className="mt-6 rounded-md border border-slate-100 bg-slate-50 p-4">
                ...
                <div className="mt-4 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                  {Object.entries(activeQuote).filter(([key]) => !["id","fullName","name","country","companyName","company","email","contactNumber","phone"].includes(key)).filter(([, value]) => value !== null && value !== undefined && value !== "").map(([key, value]) => {
                  ...
                  {Array.isArray(activeQuote.files) && activeQuote.files.length > 0 ? activeQuote.files.map((file, index) => {
                    ...
                    return (
                      <div key={`${index}-${label}`} className="rounded border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700">
                        ...
                        {fileUrl ? (fileUrl.match(/\.(png|jpe?g|gif|webp|bmp|svg)(\?|$)/i) ? <img src={fileUrl} alt={label} className="mt-2 max-h-40 rounded object-contain" /> : <a href={fileUrl} target="_blank" rel="noreferrer" className="mt-2 inline-block text-sm text-blue-600 underline">Open file</a>) : null}
                      </div>
                    );
