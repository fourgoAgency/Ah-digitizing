...
// ═══════════════════════════════════════════════════════════════════════════════
const BottomBannerLayer = ({ activeBannerIndex, visible }: { activeBannerIndex: number; visible: boolean }) => (
  <div className="fixed inset-0 z-0 pointer-events-none overflow-hidden" style={{ opacity: visible ? 1 : 0, pointerEvents: "none", transition: "opacity 0.3s" }}>
...
export default function PortfolioSection() {
  ...
  useEffect(() => {
    const update = () => {
      ...
      // Hide banner when user has scrolled past the entire portfolio section
      if (sectionEndRef.current) {
        const endTop = sectionEndRef.current.getBoundingClientRect().top;
        setBannerVisible(endTop > 0);
      } else {
        // Show banner if any spacer is in/above viewport
        setBannerVisible(next > 0 || (spacerRefs.current[0] ? spacerRefs.current[0].getBoundingClientRect().top < vh : false));
      }
    };
  ...
  return (
    <div className="relative">
      <BottomBannerLayer activeBannerIndex={activeBannerIndex} visible={bannerVisible} />
      <div className="relative z-10">
        ...
        {/* Sentinel at the very end of the portfolio section */}
        <div ref={sectionEndRef} aria-hidden="true" />
      </div>
    </div>
  );
}
