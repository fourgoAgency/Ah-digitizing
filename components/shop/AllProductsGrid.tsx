...
// ── Inner component ───────────────────────────────────────────────────────────
// useSearchParams() must be inside a <Suspense> boundary or Next.js will crash
// the build when it tries to statically prerender the page.
function GetQoutePageInner() {
  ...
  const handleContactValuesChange = useCallback(
    ({ country, phone }: { country: string; phone: string }) => {
      setFormData((prev) => {
        if (prev.country === country && prev.contactNumber === phone) return prev;
        return { ...prev, country, contactNumber: phone };
      });
      setErrors((prev) => {
  ...
  const processIncomingFiles = useCallback(
    (incomingFiles: FileList | null | undefined) => {
      const nextFiles = Array.from(incomingFiles ?? []);
      if (nextFiles.length === 0) return;

      const hasUnsafeFileName = (fileName: string) =>
        /(\.\.)|[<>:"/\\|?*\x00-\x1F]/.test(fileName);
      const videoByExtension = (fileName: string) =>
        /\.(mp4|mov|avi|wmv|mkv|webm|m4v|3gp)$/i.test(fileName);

      const blockedVideoCount = nextFiles.filter(
        (file) =>
          file.type.toLowerCase().startsWith("video/") || videoByExtension(file.name)
      ).length;
      const nonAllowedTypeCount = nextFiles.filter(
        (file) => !isAllowedUploadFile(file)
      ).length;
      const oversizedCount = nextFiles.filter(
        (file) => file.size > MAX_FILE_SIZE_BYTES
      ).length;
      const unsafeNameCount = nextFiles.filter((file) =>
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const sanitizedData = {
      ...
      width:
        validationResult.data.width?.trim() ||
        (validationResult.data.height?.trim() ? "0" : ""),
      height:
        validationResult.data.height?.trim() ||
        (validationResult.data.width?.trim() ? "0" : ""),
    };
    void sanitizedData;
  ...
  const previewWidth =
    formData.width?.trim() || (formData.height?.trim() ? "proportional" : "");
  const previewHeight =
    formData.height?.trim() || (formData.width?.trim() ? "proportional" : "");
        ...
        <section className="relative z-10 mx-auto max-w-2xl">
          <div className="rounded-2xl border border-gray-200 bg-white p-10 text-center shadow-sm">
            ...
            <p className="mt-3 text-gray-600">
              We have received your quote request. Our team will get back to you soon.
            </p>
            <button
              type="button"
      ...
      <section className="relative z-10 mx-auto max-w-7xl">
        <div className="mb-10 text-center">
          ...
          <p className="mx-auto mt-3 max-w-2xl text-gray-600">
            Tell us about your design project. Upload your files, describe your
            specifications, and we&apos;ll get back to you.
          </p>
        </div>
              ...
              <div className="space-y-4 p-5">
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <div>
                    ...
                    />
                    {errors.designName && (
                      <p className="mt-1 text-sm text-red-600">{errors.designName}</p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    <CustomDropdown
                      ...
                      onSelectAction={(selected) =>
                        emitFieldChange("numberOfColors", selected)
                      }
                    />
                    {errors.numberOfColors && (
                      <p className="mt-1 text-sm text-red-600">{errors.numberOfColors}</p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      min="0"
                      className="input"
                      placeholder="proportional to width"
              ...
              <div className="p-5">
                ...
                >
                  ...
                  <p className="mt-1 text-sm text-gray-500">
                    Accepted: JPG, PNG, JPEG, PDF, AI, EPS, SVG, PSD &amp; more
                  </p>
                  <p className="mt-1 text-sm text-gray-500">
                ...
                <span className="mt-2 block">
                  ...
                  <p className="text-sm text-gray-600">
                    Have a sample or old design you like? Upload it so we can follow the
                    same style and direction.{" "}
                    <Link
                      href="/privacy-policy#file-validation"
            ...
            <div className="overflow-hidden rounded-lg border border-gray-200 bg-white">
              ...
              <div className="p-5">
                ...
                {errors.whatsappOptIn && (
                  <p className="mt-2 text-sm text-red-600">{errors.whatsappOptIn}</p>
                )}
              </div>
            </div>
                  ...
                  <div className="rounded-md border border-dashed border-gray-300 bg-gray-50 p-4 text-center">
                    <div className="flex h-40 items-center justify-center rounded-md border border-gray-200 bg-white">
                      {previewFileUrl ? (
                        ...
                        <p className="break-all px-3 text-sm text-gray-600">
                  ...
                  <div className="space-y-2 text-left text-sm">
                    <p className="grid grid-cols-[120px_minmax(0,1fr)] gap-1">
                      ...
                      <span className="min-w-0 truncate text-gray-600">
                        {toDisplay(formData.orderType)}
                      </span>
                    </p>
                    <p className="grid grid-cols-[120px_minmax(0,1fr)] gap-1">
                      <span className="font-semibold text-gray-800">Name:</span>
                      <span className="min-w-0 truncate text-gray-600">
                        {toDisplay(formData.fullName)}
                      </span>
                    </p>
                    <p className="grid grid-cols-[120px_minmax(0,1fr)] gap-1">
                      <span className="font-semibold text-gray-800">Company:</span>
                      <span className="min-w-0 truncate text-gray-600">
                        {toDisplay(formData.companyName)}
                      </span>
                    </p>
                    <p className="grid grid-cols-[120px_minmax(0,1fr)] gap-1">
                      <span className="font-semibold text-gray-800">Contact:</span>
                      <span className="min-w-0 truncate text-gray-600">
                        {toDisplay(formData.contactNumber)}
                      </span>
                    </p>
                    <p className="grid grid-cols-[120px_minmax(0,1fr)] gap-1">
                      <span className="font-semibold text-gray-800">Email:</span>
                      <span className="min-w-0 truncate text-gray-600">
                        {toDisplay(formData.email)}
                      </span>
                    </p>
                    <p className="grid grid-cols-[120px_minmax(0,1fr)] gap-1">
                    ...
                    <p className="grid grid-cols-[120px_minmax(0,1fr)] gap-1">
                      ...
                      <span className="min-w-0 truncate text-gray-600">
                        {toDisplay(formData.designName)}
                      </span>
                    </p>
                    <p className="grid grid-cols-[120px_minmax(0,1fr)] gap-1">
                      <span className="font-semibold text-gray-800">No. of Colors:</span>
                      <span className="min-w-0 truncate text-gray-600">
                        {toDisplay(formData.numberOfColors)}
                      </span>
                    </p>
                    <div className="grid grid-cols-3 gap-1">
                      ...
                      <span className="min-w-0 truncate text-sm text-gray-600">
                        {toDisplay(formData.unitType)}
                      </span>
                      <span className="min-w-0 truncate text-sm text-gray-600">
                        {toDisplay(previewWidth)}
                      </span>
                      <span className="min-w-0 truncate text-sm text-gray-600">
                        {toDisplay(previewHeight)}
                      </span>
                    </div>
                    <p className="grid grid-cols-[120px_minmax(0,1fr)] gap-1">
                      <span className="font-semibold text-gray-800">WhatsApp:</span>
                      <span className="min-w-0 truncate text-gray-600">
                        {formData.whatsappOptIn ? "Yes" : "No"}
                      </span>
                    </p>
                    <p className="grid grid-cols-[120px_minmax(0,1fr)] gap-1">
...
// ── Default export ────────────────────────────────────────────────────────────
// <Suspense> is REQUIRED here. Without it, Next.js cannot statically prerender
// the page because useSearchParams() makes it dynamic. Suspense tells the
// build to render a static shell and defer the inner component to the client.
